...
**bf16 vs 4-bit accuracy comparison.** Cannot run bf16 on 8 GB. This is the gap I
most regret: every accuracy number here carries an unmeasured quantisation cost.

**Calibrated confidence on MLX.** `yes_probability` reads logits on the HF backend
and returns a real probability. The MLX path has no logit access wired up, so it
falls back to parsing generated text and returns only 0.0 or 1.0. Every confidence
number in these results is therefore **binary, not calibrated** — the more
interesting version of this feature is implemented but unmeasured on this machine.

**Real-screenshot evaluation.** All results are synthetic. Real UIs have
anti-aliasing, overlapping chrome, photographic content and inconsistent fonts.
Expect worse.

**Multi-image and video.** One image per call.

**Fine-tuning.** Zero-shot prompting only. A LoRA on UI grounding data would
likely close much of the coarse-vs-refine gap for free at inference, and would
still fit the budget.

**Gradio UI.** Scoped, not built. The CLI is the interface.

**Self-consistency sampling.** Would raise accuracy and multiply latency by the
sample count. On a machine already at 20s per grounding, zoom-refine buys more
accuracy per second. Deliberate, not overlooked.

## Known failure modes

- Refine cannot fix a coarse pass that found the *wrong* element; it confirms the
  wrong thing at higher resolution. `search_tiles` mitigates this at 4x latency.
- Targets under ~10px tall are unreliable even after zooming.
- The model sometimes echoes the requested target string back as its label
  instead of describing what it found.
- `diff` composes two screenshots side by side into one image, so each panel gets
  roughly half the pixel budget. Fine for small UI deltas, poor for two dense
  high-resolution screens.
- `redact` is a VLM reading pixels, not a validated PII detector. It will miss
  unusual formats and flag non-sensitive values. First pass for a human to review,
  never a compliance control.
